...
void printAttributes(FaceBox box) {
    ...
    printf("Liveness: %f\n", box.liveness);
    printf("Age: %d\nGender: %s\n", box.age, (box.gender == 0) ? "Male" : "Female");
    ...
    printf("Mouth Open: %s(%f)\n", (box.mouth_opened > 0.5) ? "Open" : "Close", box.mouth_opened);
    printf("Face Occlusion: %s(%f)\n", (box.face_occlusion > 0.5) ? "Yes" : "No", box.face_occlusion);
    printf("Face Quality: %f, Face Luminance: %f\n", box.face_quality, box.face_luminance);
}
...
int main()
{
    ...
    const char* hwid = ttv_get_hwid(LOCAL);
    printf("hwid: %s\n", hwid);
    ...
    ret = ttv_detect_face(image1, faceBoxes1, false, true, true, true, true);
    ...
    ret = ttv_detect_face(image2, faceBoxes2, false, true, true, true, true);
